#!/bin/bash
# Insall Common Tools
sudo yum -y install tcpdump telnet bind-utils zip unzip tcpdump telnet bind-utils wget -yum
curl "http://s.amazonaws.com/aws-cli/awscli-bundle.zip" -o "awscli-bundle.zip"
...
cat >> /etc/fstab << EOF
$EFS_PATH_SSH:/$GITLAB_DIR/.ssh nfs nfsvers=4.1,rsize=1048576,wsize=1048576,timeo=600,retrans=2,noresvport,_netdev 0 0 
$EFS_PATH_UPLOADS:/$GITLAB_DIR/gitlab-rails/uploads nfs nfsvers=4.1,rsize=1048576,wsize=1048576,timeo=600,retrans=2,noresvport,_netdev 0 0 
$EFS_PATH_SHARED:/$GITLAB_DIR/gitlab-rails/shared nfs nfsvers=4.1,rsize=1048576,wsize=1048576,timeo=600,retrans=2,noresvport,_netdev 0 0 
$EFS_PATH_BUILDS:/$GITLAB_DIR/gitlab-ci/builds nfs nfsvers=4.1,rsize=1048576,wsize=1048576,timeo=600,retrans=2,noresvport,_netdev 0 0 
$EFS_PATH_DATA:/$GITLAB_DIR/git-data nfs nfsvers=4.1,rsize=1048576,wsize=1048576,timeo=600,retrans=2,noresvport,_netdev 0 0 
EOF

mount -a

URL='aws elbv2 --region us-east-2 --output tex efs describe-load-balancers | grep private-apps|awk '{print $4}'
export URL
EXTERNAL_URL="http://$URL";export EXTERNAL_URL

sudo yum install -y gitlab-ce-11.5.6-ce.0.el6
